...
public class DefaultNotesService implements NotesService {
    private static final Logger log = LoggerFactory.getLogger(DefaultNotesService.class);
    @Autowired
...
//    }
    ...
    private NoteEntry toNoteEntry(Row row, String encryptionKey) {
        NoteEntry noteEntry = new NoteEntry(row.getString("notebook"), row.getString("noteid"),  EncryptionUtil.decrypt(encryptionKey, row.getString("key")),
                EncryptionUtil.decrypt(encryptionKey, row.getString("value")), EncryptionUtil.decrypt(encryptionKey, row.getString("info")), row.getBool("isPassword"),
                LocalDateTime.ofInstant(row.getTimestamp("lastModifiedTime").toInstant(), ZoneOffset.UTC));
        return noteEntry;
    }
...
//        session.execute(SimpleStatement.builder(cqlStr).addPositionalValues(notebook, noteIds).build());
        repository.deleteNotes(userId, noteIds);
    }
